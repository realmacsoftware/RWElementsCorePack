...
const advancedClasses = (rw) => {
  ...
  const {
    ...
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
  } = globalBgImageFetchPriority(rw), classes = {
    wrapper: classnames([
      "flex",
      flexDirection,
      flexAlignItems,
    ...
    item: classnames([
      ...
      globalTransitions(rw, !0),
      globalMenuItem(rw)
    ]).toString(),
    icon: classnames([iconSize, iconSpacing]).toString()
  }, processPages = (pages2) => pages2.filter(
    (page) => page.displayInMenu && !page.isDraft && !page.isFolder
  ).map((page) => {
    const processedSubPages = page.pages ? processPages(page.pages) : null, { icon } = page, isSvgIcon = (icon == null ? void 0 : icon.format) === "svg", hasIcon = !!(icon != null && icon.format), shouldShowIcon = showIcons && hasIcon, processedIconSvg = isSvgIcon && icon.image ? icon.image.includes('class="') ? icon.image.replace(
      /class="[^"]*"/g,
      `class="${classes.icon}"`
    ) : icon.image.replace(
      /<svg/,
    ...
    return {
      ...
      hasPages: !!(processedSubPages != null && processedSubPages.length),
      iconIsSvg: isSvgIcon,
      hasIcon,
    ...
    if (show === "all")
      return rw.pages;
    if (!(folderLink != null && folderLink.title))
      return [];
    const findFolder = (pages2, targetTitle) => {
      ...
      return null;
    }, targetFolder = findFolder(rw.pages, folderLink.title);
    return (targetFolder == null ? void 0 : targetFolder.pages) || [];
  })(), pages = processPages(wantedPages);
  rw.setRootElement({
