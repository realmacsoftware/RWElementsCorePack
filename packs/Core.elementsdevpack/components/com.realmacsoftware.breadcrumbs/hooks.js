...
const transformHook = (rw) => {
  ...
  const buildBreadcrumbs = (pages, trail2 = []) => {
  ...
  const wantsTruncate = truncateOnMobile === true || truncateOnMobile === "true";
  ...
  const defaultChevron = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" /></svg>';
  const sizeValue = separatorSize ? separatorSize.replace("size-", "") : "4";
  const svgSizeClasses = `w-${sizeValue} h-${sizeValue}`;
  const hasCustomSeparator = (separatorIcon == null ? void 0 : separatorIcon.image) && separatorIcon.image.includes("<svg");
  ...
  const separatorContent = hasCustomSeparator ? injectSvgClasses(separatorIcon.image) : injectSvgClasses(defaultChevron);
  const classes = {
    wrapper: classnames([
      "flex flex-wrap",
      flexAlignItems,
