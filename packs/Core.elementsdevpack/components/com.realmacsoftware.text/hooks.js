...
const transformHook = (rw) => {
  ...
  const linkModifiers = Object.entries({
    ...
    linkUnderline,
    linkFontWeight
  }).map(([, value]) => injectPrefixOnDarkModeColors("[&_a]", `[&_a]:${value}`)).join(" ");
  const linkModifiersHover = Object.entries({
    linkColorHover,
    linkUnderlineHover,
    linkFontWeightHover
  }).map(
    ([, value]) => injectPrefixOnDarkModeColors("[&_a:hover]", `[&_a:hover]:${value}`)
  ).join(" ");
  const linkModifiersVisited = Object.entries({
    ...
    ([, value]) => injectPrefixOnDarkModeColors("[&_a:visited]", `[&_a:visited]:${value}`)
  ).join(" ");
  const classes = classnames([
    linkModifiers,
    linkModifiersHover,
  ...
  const rootElementTag = typeof tag === "string" ? tag === "custom" ? customTag : tag : "p";
  rw.setRootElement({
    as: rootElementTag,
