...
const overlay = (rw) => {
  ...
  const {
  ...
  } = rw.props, { mode } = rw.project, {
    globalBgImageFetchPriorityEnabled,
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
  } = globalBgImageFetchPriority(rw), linkModifiers = Object.entries({
    linkColor,
    linkUnderline,
    linkFontWeight
  }).map(([, value]) => `[&_a]:${value}`).join(" "), linkModifiersHover = Object.entries({
    linkColorHover,
    linkUnderlineHover,
    linkFontWeightHover
  }).map(([, value]) => `[&_a:hover]:${value}`).join(" "), linkModifiersVisited = Object.entries({
    linkColorVisited,
    linkUnderlineVisited,
    linkFontWeightVisited
  }).map(([, value]) => `[&_a:visited]:${value}`).join(" "), classes = classnames([
    linkModifiers,
    linkModifiersHover,
    ...
    globalEffects(rw),
    globalTransforms(rw),
    mode === "edit" ? classnames(
      globalTextFontsAndTextStyles(rw).split(" ").filter(Boolean)
    ).modifier("[&_[data-slate-node]]").toString() : globalTextFontsAndTextStyles(rw),
    globalBorders(rw),
    headingTextAlign
  ]), editClasses = classnames(classes.toString()).modifier("").toString(), rootElementTag = typeof tag == "string" ? tag === "custom" ? customTag : tag : "p";
  rw.setRootElement({
    as: rootElementTag,
    class: mode === "edit" ? classnames([editClasses]).toString() : classes.toString(),
    args: {
      id: globalID
    }
