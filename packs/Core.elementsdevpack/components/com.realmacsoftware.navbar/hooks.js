...
const advancedClasses = (rw) => {
  ...
  const {
    ...
    mobileBackdropBlur
  } = rw.props, { project } = rw, { logo, title } = project, { id } = rw.node, {
    globalBgImageFetchPriorityEnabled,
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
  } = globalBgImageFetchPriority(rw), hasLogoLink = logoLink.href.length > 0, filterPages = (pages2) => pages2.filter((page) => page.displayInMenu && !page.isDraft).map((page) => {
    const filteredPages = page.pages ? filterPages(page.pages) : null;
    return {
      ...page,
      pages: filteredPages,
      hasPages: filteredPages ? filteredPages.length > 0 : !1
    };
  }), pages = filterPages(rw.pages), hasLogo = !!logo.url, wantsLogo = logoType == "logo", wantsTitle = logoType == "title", contentWidthProps = {
    props: {
      ...rw.props,
  ...
  logoLink.href || (logoLink.href = project.siteUrl || "/"), logoLink.title || (logoLink.title = project.title), logoLink.target || (logoLink.target = "_self");
  const deviceModifierForMobileTrigger = mobileTriggerDisplay ? `${mobileTriggerDisplay}:` : "";
  ...
  const classes = {
    ...
    logoWrapper: classnames([
      "flex",
      mobileTriggerPosition == "first" && `order-last ${deviceModifierForMobileTrigger}order-first`,
      itemsAlignment != "start" && `${deviceModifierForMobileTrigger}flex-1`,
      logoMargin
    ]).toString(),
    ...
    desktop: {
      ...
      item: classnames([
        ...
        globalNavItems(rw),
        globalTransitions(rw, { isContainer: !0 })
      ]).toString(),
      ...
      rightItems: classnames([
        ...
        itemsAlignment != "end" && `${deviceModifierForMobileTrigger}flex-1`
      ]).toString(),
      subMenu: classnames([
        "w-max",
        mobileMenuBackground,
    ...
    mobile: {
      menuButton: classnames([
        "flex",
        deviceModifierForMobileTrigger.length ? `${deviceModifierForMobileTrigger}hidden` : ""
      ]).toString(),
      trigger: classnames([
        "cursor-pointer",
        mobileTriggerColor,
      ...
      closeTrigger: classnames([
        ...
        mobileTriggerColor,
        mobileTriggerColorHover,
        mobileTriggerPosition == "first" && `order-first ${deviceModifierForMobileTrigger}order-last`,
        mobileCloseTop,
        mobileCloseRight,
        mobileCloseLeft
      ]).toString(),
  ...
  rw.setRootElement({
    ...
    logoLink,
    project,
    mobileBackdropTransitionAttributes: getAlpineTransitionAttributesMobile("fade"),
    mobileTransitionAttributes: getAlpineTransitionAttributesMobile(menuAnimation),
    desktopTransitionAttributes: getAlpineTransitionAttributesDesktop(menuAnimation),
    mobileMenuId: `mobile-menu-${id}`,
    mobileMenuPreview: mobileMenuPreview == "true" || mobileMenuPreview == "true",
    includeMobileMenu: mobileMenuPreview == "true" || rw.project.mode != "edit",
