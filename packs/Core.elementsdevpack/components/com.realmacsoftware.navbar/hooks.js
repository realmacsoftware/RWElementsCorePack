...
const transformHook = (rw) => {
  ...
  const filterPages = (pages2) => {
    return pages2.filter((page) => page.displayInMenu && !page.isDraft).map((page) => {
  ...
  const menuItemBackgroundClasses = classnames([
    ...
    globalNavItemsBackgroundTypeHover == "color" && globalNavItemsBackgroundColorHover,
    globalNavItemsBackgroundTypeHover == "color" && globalNavItemsBackgroundOpacityHover,
    globalNavItemsBackgroundPadding,
    globalNavItemsBackgroundRadius
  ]).toString();
  const classes = {
