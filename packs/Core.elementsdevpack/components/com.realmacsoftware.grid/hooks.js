...
function getResponsiveColumns(columnsProp, names) {
  const deviceOrder = ["base", "sm", "md", "lg", "xl", "2xl"], deviceColumns = deviceOrder.reduce((acc, key, idx) => {
    var _a;
    const prevKey = deviceOrder[idx - 1];
    return acc[key] = (_a = columnsProp[key]) != null ? _a : prevKey ? acc[prevKey] : 0, acc;
  }, {});
  return deviceOrder.map((key, idx) => {
    const columnCount = deviceColumns[key], visibilityClasses = deviceOrder.map((otherKey, otherIdx) => otherKey === key ? otherKey === "base" ? "grid" : `${key}:grid` : otherKey === "base" ? "hidden" : `${otherKey}:hidden`), gridClass = key === "base" ? `grid-cols-${columnCount}` : `${key}:grid-cols-${columnCount}`;
    return {
      device: key,
      columnCount,
...
function getResponsiveRows(rowsProp, names) {
  const deviceOrder = ["base", "sm", "md", "lg", "xl", "2xl"], deviceRows = deviceOrder.reduce((acc, key, idx) => {
    var _a;
    const prevKey = deviceOrder[idx - 1];
    return acc[key] = (_a = rowsProp[key]) != null ? _a : prevKey ? acc[prevKey] : 0, acc;
  }, {});
  return deviceOrder.map((key, idx) => {
    const rowCount = deviceRows[key], visibilityClasses = deviceOrder.map((otherKey, otherIdx) => otherKey === key ? otherKey === "base" ? "grid" : `${key}:grid` : otherKey === "base" ? "hidden" : `${otherKey}:hidden`), gridClass = key === "base" ? `grid-rows-${rowCount}` : `${key}:grid-rows-${rowCount}`;
    return {
      device: key,
      rowCount,
...
const transformHook = (rw) => {
  ...
  const {
    ...
    alignItems,
    alignContent
  } = rw.props, { columns: columnsProp, rows: rowsProp } = rw.responsiveProps, { mode } = rw.project, { id } = rw.node, { names } = rw.theme.breakpoints, wantsHorizontalDropzone = columns > 1, link = globalLink(rw), filter = globalFilter(rw), {
    globalBgImageFetchPriorityEnabled,
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
  } = globalBgImageFetchPriority(rw), responsiveColumns = getResponsiveColumns(columnsProp, names), responsiveRows = getResponsiveRows(rowsProp || {}, names), classes = [
    `group/${id} group/grid`,
