...
const overlay = (rw) => {
  ...
  const {
    ...
    controls,
    globalPadding
  } = rw.props, { id } = rw.node, { assetPath } = rw.component, hasThumbnail = thumbnail, hasDarkThumbnail = thumbnailDark, options = {
    autoplay,
    loop,
    muted,
    controls,
  ...
  (video == null ? void 0 : video.format) == "mp4" && (video.image = `${assetPath}/video-placeholder.png`);
  const finalVideo = video || {
    ...
    globalFilters(rw),
    globalBorders(rw)
  ]).toString(), videoClasses = classnames([
    "aspect-video w-full h-auto",
    objectClasses(rw),
    globalPadding
  ]).toString(), videoLightboxClasses = classnames([
    "aspect-video w-[min(95vw,calc(95vh*16/9))] max-w-[1920px] max-h-[95vh]",
    objectClasses(rw),
    globalPadding
  ]).toString(), posterClasess = classnames([
    "absolute inset-0 z-0 cursor-pointer w-full h-full",
    globalPadding,
    video ? "" : "object-cover",
    objectClasses(rw)
  ]).toString(), overlayClasses2 = classnames([
  ...
  }, getOptions = () => JSON.stringify(options).replace(/"/g, "'"), getXData = () => `videoPlayer('${id}', '${finalVideo.format}', '${finalVideo.videoId}', ${getOptions()})`;
  rw.setRootElement({
    as: "div",
    ...
    thumbnail,
    thumbnailDark,
    thumbnailAlt: thumbnailAlt || (video == null ? void 0 : video.name),
