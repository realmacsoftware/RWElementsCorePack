...
const advancedClasses = (rw) => {
  ...
  const {
  ...
  } = rw.props, hasSvgIcon = (svgIcon == null ? void 0 : svgIcon.format) === "svg", slugify = (value) => String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""), openPages = [], findActivePage = (pages2) => {
    if (!Array.isArray(pages2)) return null;
    ...
    return null;
  }, buildPages = (pages2, parentPath = "", relativeLevel = 0) => {
    const result = [];
    if (!Array.isArray(pages2) || pages2.length === 0) return result;
    const hasReachedMaxDepth = maxLevelsDepth === "manual" && maxLevels > 0 && relativeLevel >= maxLevels;
    for (let i = 0; i < pages2.length; i += 1) {
      const page = pages2[i];
      if (!(page.displayInMenu && !page.isDraft)) continue;
      const slug = slugify(page.title), id = parentPath ? parentPath + "-" + slug : slug, children = hasReachedMaxDepth ? [] : buildPages(page.pages || [], id, relativeLevel + 1), hasPages = children.length > 0, hasActiveChild = hasPages && children.some((c) => c.isActive || c.hasActiveChild), processedPage = {
        ...page,
        id,
        pages: children,
        hasPages,
      ...
      (processedPage.isActive || hasActiveChild) && openPages.push(id), result.push(processedPage);
    }
    return result;
  }, wantsAllPages = wantedPages === "all", wantsActivePages = wantedPages === "active", wantsSelectedPage = wantedPages === "manual", findPageByTitle = (pages2, targetTitle) => {
    var _a;
    for (const page of pages2 || []) {
    ...
    if (wantsSelectedPage && wantedPagesPageLink) {
      const folderPage = findPageByTitle(
        rw.pages,
        wantedPagesPageLink == null ? void 0 : wantedPagesPageLink.title
      );
  ...
  rw.setRootElement({
    ...
    pages,
    showSubMenus: rw.project.mode !== "edit" ? !0 : previewSubMenus,
