...
const advancedClasses = (rw) => {
  ...
  const {
    ...
    iconColorClosed,
    globalFilterGroup
  } = rw.props, { id, parent } = rw.node, { tags } = rw.collections, dataTags = (tags == null ? void 0 : tags.map((tag) => tag.title).join(",")) || "", hasAnIcon = icon && icon.format === "svg", showContent = rw.project.mode != "edit" ? !0 : showContentInEdit, filter = globalFilter(rw), {
    globalBgImageFetchPriorityEnabled,
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
