...
const advancedClasses = (rw) => {
  ...
  const {
    ...
    background: classnames([
      ...
      angleClass,
      bgFixed && "fixed [will-change:transform] [transform:translateZ(0)]",
      !bgFixed && "absolute",
      globalBackground(rw)
    ]).toString()
  };
  globalID.length > 0 && (console.log("adding anchor", globalID), rw.addAnchor(globalID)), rw.setRootElement({
    as: "div",
...
function setSVGClass(svgString, newClassValue) {
  return svgString && (svgString.match(/class="[^"]*"/) ? svgString.replace(/class="[^"]*"/, `class="${newClassValue}"`) : svgString.replace(/<svg([^>]*)/, `<svg$1 class="${newClassValue}"`));
}
