...
const overlay = (rw) => {
  ...
  } = rw.props, { screens } = rw.theme.breakpoints, { id } = rw.node, hasResources = ((_a = resources == null ? void 0 : resources.resources) == null ? void 0 : _a.length) > 0;
  (_b = resources == null ? void 0 : resources.resources) == null || _b.forEach((resource) => {
    resource.srcset = "", resource.thumbnail = rw.resizeResource(resource, 400), resource.alt = resource.caption || resource.author || "", resource.isVideo = resource.format === "youtube" || resource.format === "vimeo" || resource.format === "mp4", resource.isVideo && (resource.isYouTube = resource.format === "youtube", resource.isVimeo = resource.format === "vimeo", resource.isMP4 = resource.format === "mp4", resource.options = {}, resource.caption = resource.name, resource.author = resources.name, resource.isYouTube && (resource.options = {
      autoplay: 0,
      loop: 0,
      muted: 0,
      controls: 1
    }), resource.isVimeo && (resource.options = {
      autoplay: "false",
      loop: "false",
      muted: "false",
      controls: "true"
