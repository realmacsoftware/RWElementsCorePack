...
const overlay = (rw) => {
  ...
  const {
  ...
  } = rw.props, link = globalLink(rw), hasSVG = svg == null ? void 0 : svg.image, classes = classnames([
    globalSizing(rw),
    globalLayout(rw),
    globalSpacing(rw),
    ...
    globalFilters(rw),
    advancedClasses(rw)
  ]).toString(), { id: parentID } = rw.node.parent, fillColorPrefix = fillColorHoverElement == "self" ? "hover" : `group-hover/${parentID}`, strokeColorPrefix = strokeColorHoverElement == "self" ? "hover" : `group-hover/${parentID}`, svgClasses = classnames([
    globalSizing(rw),
    globalTransitions(rw),
    ...
    return stripFill && (cleaned = cleaned.replace(
      /fill="(?!none")[^"]*"/g,
      'fill="currentColor"'
    )), stripStroke && (cleaned = cleaned.replace(/stroke-width="[^"]*"/g, "")), stripSize && (cleaned = cleaned.replace(/(?<!stroke-)width="[^"]*"/g, "").replace(/\bheight="[^"]*"/g, "")), stripStyles && (cleaned = cleaned.replace(/style="[^"]*"/g, "")), cleaned = cleaned.includes("<svg") ? cleaned.replace(/<svg([^>]*)>/g, (match, attributes) => {
      const existingClass = attributes.match(/class="([^"]*)"/), existingClasses = existingClass ? existingClass[1] + " " : "";
      return match.replace(/class="[^"]*"/, "").replace(
        "<svg",
        `<svg class="${existingClasses}${svgClasses}"`
      );
    }) : cleaned, cleaned;
  }, placeHolderSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="${svgClasses}">
