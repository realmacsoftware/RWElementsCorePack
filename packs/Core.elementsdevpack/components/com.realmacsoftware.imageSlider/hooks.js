...
const overlay = (rw) => {
  ...
  const {
    ...
    paginationDotSize,
    paginationDotGap
  } = rw.props, visibleSlides = transitionType === "fade" ? "1" : rawVisibleSlides, { id } = rw.node, isCms = imageType === "cms", cmsField = imageCmsField === "custom" ? imageCmsCustomField : imageCmsField, { assetPath } = rw.component, hasImages = isCms || (images == null ? void 0 : images.resourceCount) > 0, resources = isCms ? {
    resources: [
      {
  ...
  } : images, gridColumns = enablePaginationArrows ? "grid-cols-[auto_1fr_auto]" : "grid-cols-1", gridRows = enablePaginationDots ? "grid-rows-[1fr_auto]" : "grid-rows-1", widths = {
    1: "w-full",
    2: "w-1/2",
    3: "w-1/3",
    4: "w-1/4",
    ...
    11: "w-[9.090909%]",
    12: "w-[8.333333%]"
  }, isFade = transitionType === "fade", aspectRatioClass = aspectRatio === "aspect-[custom]" ? customAspectRatio : aspectRatio, hasFixedAspectRatio = aspectRatio !== "aspect-[auto]", classes = {
  ...
  }, sliderOptions = JSON.stringify({
    visibleSlides: parseInt(visibleSlides),
    autoplay: autoPlay,
    autoplayDelay: autoPlayDelay,
    transitionType: transitionType || "slide",
    transitionDuration: transitionDuration || "duration-[500ms]"
  }).replace(/"/g, "'"), slides = (_a = resources == null ? void 0 : resources.resources) == null ? void 0 : _a.map((resource) => `'${resource.image}'`).join(","), rootArgs = {
  ...
  pauseOnHover && (rootArgs["x-on:mouseenter"] = "stopAutoplay", rootArgs["x-on:mouseleave"] = "options.autoplay && startAutoplay()"), rw.setRootElement({
