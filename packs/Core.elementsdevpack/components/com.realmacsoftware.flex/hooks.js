...
const overlay = (rw) => {
  ...
  const {
    ...
    alignContent,
    justifyContent
  } = rw.props, { id } = rw.node, wantsHorizontalDropzone = flexDirection.includes("row"), link = globalLink(rw), filter = globalFilter(rw), {
    globalBgImageFetchPriorityEnabled,
    globalBgImageFetchPriorityLinkElement,
    globalBgImageFetchPriorityLinkElementEnd
  } = globalBgImageFetchPriority(rw), classNames = classnames([
